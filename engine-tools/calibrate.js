...
function play(lineA, lineB, games, emptyAs = 'average', fillers) {
  const A = SIMKIT.buildSide(0, lineA, { emptyAs, fillers }), B = SIMKIT.buildSide(1, lineB, { emptyAs, fillers });
...
if (!ONLY_WINS) {
  // 1. ref
  const base = play({}, {}, Math.max(2000, N * 10), 'average', 'engine');
