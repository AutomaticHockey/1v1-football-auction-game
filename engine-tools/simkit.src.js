...
const SIMKIT = (() => {
  ...
  const K = {
    ...
    grade: {
      ref: { ypc: 4.3, rbYpr: 7.217, wrCatch: 0.619, wrYpr: 12.587, cmp: 0.617, qbYds: 246, int: 0.78, defYds: 326, take: 1.481 },
      run: 0.82,          // yards a carry per yard of graded yards per carry over ref
    ...
    // RB and WR 60% (a star FLEX against an empty one, 57-60%).
    balance: { QB: 0.63, RB: 1.1, WR: 0.65, TE: 0.65, DEF: 0.55 }
  };
  ...
  // A real player's or defense's edges from his grade: { run, catch, yards, int }, neutral 0, 0, 1, 1,
  // scaled by the slot's balance (additive edges times it, factors to its power).
  function edgeFor(pos, g) {
    const e = rawEdge(pos, g), b = K.balance[pos];
    if (!e || b === 1) return e;
    return { run: b * e.run, catch: b * e.catch, yards: Math.pow(e.yards, b), int: Math.pow(e.int, b) };
  }
  ...
  function rawEdge(pos, g) {
    const G = K.grade, R = G.ref, e = { run: 0, catch: 0, yards: 1, int: 1 };
    if (!g) return null;
    if (pos === 'RB') { e.run = G.run * (g.ypc - R.ypc); e.yards = Math.pow(g.ypr / R.rbYpr, G.rbCatchYards); }
    if (pos === 'WR' || pos === 'TE') { e.catch = G.catch * (g.catch - R.wrCatch); e.yards = Math.pow(g.ypr / R.wrYpr, G.catchYards); }
    if (pos === 'QB') {
